...
//=====================================================================
	...
	void PolyMesh::deleteEdge(MEdge* edge)
	{
		assert(edge != nullptr);
		MHalfedge* he0 = edge->halfEdge();
		MHalfedge* he1 = edge->halfEdge()->pair();
	...
	size_t PolyMesh::delete_isolated_edge()
	{
...
//=====================================================================
	...
	MEdge* PolyMesh::splitFaceMakeEdge(MPolyFace* face, MVert* v0, MVert* v1)
	{
		...
		std::vector<MHalfedge*> f0_loop; f0_loop.clear();
		std::vector<MHalfedge*> f1_loop; f1_loop.clear();

		he_begin = he0; he = he0;
		do {
			f0_loop.push_back(he);
			he = he->next();
		} while (he == he1);

		he_begin = he1; he = he1;
		do {
			f1_loop.push_back(he);
			he = he->next();
		} while (he == he0);
		...
		f1_loop.push_back(he1_new);
		MPolyFace* f_new = newPolyFace();
		f_new->setHalfedge(he1);
		for (MHalfedge* lhe : f1_loop)
		{
			lhe->setPolygon(f_new);
		}
	...
	void PolyMesh::updateVerticesNormal()
	{
		for (size_t i = 0; i < vertices_.size(); i++)
		{
